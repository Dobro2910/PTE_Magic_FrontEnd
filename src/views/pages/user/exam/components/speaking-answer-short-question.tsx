...
import { TIME_PREPARE_AUDIO, TIME_PREPARE_RECORDER, TIME_LENGTH_RECORDER, ANIMATION_IN, ANIMATION_OUT, EVENT, EXAM_GROUP, APP_LOCAL_DATETIME_FORMAT, RECORDED_FILE_EXTENSION } from 'src/config/constants';
...
export default class SpeakingASQ extends React.Component<any, any> {
  constructor(props) {
    super(props);
  ...
  componentWillUnmount() {
    ...
    console.log('=== ASQ componentWillUnmount ===');
  ...
  onTriggerRecord = () => {
    ...
    this.pteRecorderAnswerRef.current.triggerStartRecord();
  };

  onCompletedQuestion = () => {
    // tslint:disable-next-line
    console.log('onCompletedQuestion speaking answer short question');
    this.onAnswer();
  };
      ...
      <div>
        <Animated animationIn={ANIMATION_IN} animationOut={ANIMATION_OUT} isVisible>
          <PteAudioPlayer
            ...
            delay={TIME_PREPARE_AUDIO.SPEAKING_ANSWER_SHORT_QUESTION}
            onCompleted={this.onCompletedAudio}
            examGroup={this.props.examGroup}
          />
          <PteRecorder
            ref={this.pteRecorderAnswerRef}
            delay={TIME_PREPARE_RECORDER.SPEAKING_ANSWER_SHORT_QUESTION}
            length={TIME_LENGTH_RECORDER.SPEAKING_ANSWER_SHORT_QUESTION}
            recording={recording}
