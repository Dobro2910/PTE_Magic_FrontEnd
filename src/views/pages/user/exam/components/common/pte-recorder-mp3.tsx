...
export interface IPteRecorderProps {
  delay: number;
  ...
  examGroup?: string;
  onProcessRecordData?: Function;
}

export interface IPteRecorderState {
  status: any;
  // audioType: string;
  audioSrc: string;
  started: boolean;
  startedDelay: boolean;
  statusRecord: string;
  progress: number;
  transcript?: string;
...
export class PteRecorder1 extends React.Component<IPteRecorderProps, IPteRecorderState> {
  constructor(props) {
    super(props);
  ...
  getAnswer = () => {
    ...
    //   );
    // });
    // // let answer = this.state.audioSrc;
    // return answer;
  };
                      ...
                      { this.props.recording && startedDelay &&
                        ...
                        <>
                          <Timer active onFinish={this.onCountdownFinish} 
                            ...
                            { duration && time && 
                              <><span>Begining in { Math.round((duration - time)/1000) } seconds</span> </>
                            }
                        </>
                      }
